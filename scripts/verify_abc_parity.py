...
def _augment_sys_path() -> None:
    # Order matters: REPO_ROOT must come first so OUR plugins package
    # resolves ahead of hermes-upstream's plugins/ (which is a regular
    # package and would otherwise shadow ours).
...
def check_memory_provider() -> list[str]:
    ...
    for method_name, kw_only_param, expect_kwargs in hook_checks:
        ...
        if kw_only_param and not _is_keyword_only_with_default(method, kw_only_param):
            ...
            if method_name in vars(SonzaiMemoryProvider):
        ...
        if method_name == "on_pre_compress" and method_name in vars(SonzaiMemoryProvider):
            try:
                ret = method([])
    ...
    if not issubclass(SonzaiMemoryProvider, MemoryProvider):
        print(
...
def check_context_engine() -> list[str]:
    ...
    for method_name, flags in overrides:
        if method_name not in vars(SonzaiContextEngine):
            continue
        method = getattr(instance, method_name)
        sig = _signature(method)
        notes: list[str] = []
    ...
    if not issubclass(SonzaiContextEngine, ContextEngine):
        print(
